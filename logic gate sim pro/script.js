...
function addGate(e, loadedGateInfo=-1) {
	...
	let gateObject = {
		...
		removeConnection: function(deletedGateID) {
			...
			while (connectionsToDelete.length > 0) {
				this.connections.splice(connectionsToDelete[0], 1);
	...
	world.gates["gate" + world.uniqueGateId] = gateObject;
	...
	window.addEventListener("mousemove", e => {
		...
		updateGateLocation(world.draggedGates, e);
		drawWiresToScreen();
	});
	...
	window.addEventListener("touchmove", e => {
		if (world.draggedGates == -1) return;
		updateGateLocation(world.draggedGates, e.touches[0]);
		drawWiresToScreen();
	});
...
function updateGateLocation(gateID, e) {
	world.gates[gateID].element.style.left = (e.pageX) + "px";
	world.gates[gateID].element.style.top = (e.pageY) + "px";

	if (world.gates[gateID].x != e.pageX || world.gates[gateID].y != e.pageY) world.mouseMovedDuringDrag = true;

	world.gates[gateID].x = e.pageX;
	world.gates[gateID].y = e.pageY;
	...
	for (let i = 0; i < world.gates[gateID].maxInputs; i++) {
		...
		world.gates[gateID].inputElements[i].style.left = (e.pageX - 45) + "px";
		world.gates[gateID].inputElements[i].style.top = (e.pageY + (25 * i) + centeringConst) + "px";
	}
	...
	for (let i = 0; i < world.gates[gateID].maxOutputs; i++) {
		...
		world.gates[gateID].outputElements[i].style.left = (e.pageX + gateWidth + 5) + "px";
		world.gates[gateID].outputElements[i].style.top = (e.pageY + (25 * i) + centeringConst) + "px";
	}

	world.gates[gateID].deleteButton.style.left = (e.pageX - 8) + "px";
	world.gates[gateID].deleteButton.style.top = (e.pageY + 5 + gateHeight) + "px";
}
...
function addTextBox(buttonEntered, e, textboxText="Click to edit") {
	if (world.draggedGates == -1 && buttonEntered == 2) {
		...
		texBoxDelete.addEventListener("click", e => {
			...
			document.body.removeChild( world.textBoxes["texbox" + tempTexboxId.toString()].element );
		...
		window.addEventListener("mousemove", e => {
			...
			textBoxes[world.draggedTextbox].x = (e.pageX - 10);
			textBoxes[world.draggedTextbox].y = (e.pageY - 10);
		})
		...
		window.addEventListener("touchmove", e => {
			...
			let textItem = textBoxes[world.draggedTextbox];
			textItem.element.style.left = (e.touches[0].pageX - 10) + "px";
			textItem.element.style.top = (e.touches[0].pageY - 10) + "px";

			textBoxes[world.draggedTextbox].x = (e.touches[0].pageX - 10);
			textBoxes[world.draggedTextbox].y = (e.touches[0].pageY - 10);
		})
...
function moveEditorByDragging(e) {
	...
	// Reposition all the gates
	for (const gateKey in world.gates) {

		let item = world.gates[gateKey];

		newXLocation = Number(item.element.style.left.substring(0, item.element.style.left.length - 2)) + scroll.changeX;
		newYLocation = Number(item.element.style.top.substring(0, item.element.style.top.length - 2)) + scroll.changeY;
...
document.getElementById("loadFileInput").addEventListener("change", e => {
	if (!confirm("Loading this file will delete your current workspace, are you sure you want to continue?")) return;
...
function loadSaveData(savedJSON) {
	...
	for (const itemKey in tempWorld.gates) {
		...
		world.gates["gate" + tempWorld.gates[itemKey].id].element.style.left = (tempWorld.gates[itemKey].x) + "px";
		world.gates["gate" + tempWorld.gates[itemKey].id].element.style.top = (tempWorld.gates[itemKey].y) + "px";
	}
