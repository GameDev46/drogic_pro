...
function addGate(e, loadedGateInfo=-1) {
	...
	let gateObject = {
		...
		run: function(isPulse=false) {
			...
			if (this.type == "decimal") {
				...
				if (!isPulse) this.ramMemory["dec"] = Number(this.infoBox.children[0].value);

				let binaryString = Number( Math.min( Number(str), 255 ) ).toString(2);
				...
				for (let i = 0; i < 8; i++) {
					if(this.ramMemory["bit" + i]) outBinaryString = "1" + outBinaryString;
					if(!this.ramMemory["bit" + i]) outBinaryString = "0" + outBinaryString;
				}

				this.infoBox.children[1].innerText = outBinaryString
			}
	...
	if (gateObject.type == "decimal") {
		...
		infoBoxInput.addEventListener("change", e => {
			...
			world.gates["gate" + parentGateID].run();
		})

		let infoBoxBinDisplay = document.createElement("button");
		infoBoxBinDisplay.innerText = "00000000";
		infoBox.appendChild(infoBoxBinDisplay);
	}
	...
	if (gateObject.type == "screen") {
		...
		gateObject.element.style.width = "400px";
		gateObject.element.style.minWidth = "400px";
		gateObject.element.style.height = "450px";
		gateObject.element.style.minHeight = "450px";
	...
	if (gateObject.type == "alu" || gateObject.type == "ram" || gateObject.type == "8BitLatch" || gateObject.type == "decimal" || gateObject.type == "oscillator") {
		gateObject.deleteButton.style.width = "150px";
		gateObject.deleteButton.style.minWidth = "150px";
	}

	if (gateObject.type == "screen") {
		gateObject.deleteButton.style.width = "400px";
		gateObject.deleteButton.style.minWidth = "400px";
	}
